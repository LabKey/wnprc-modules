SELECT p.objectid
      ...
      ,p.sireid
      ,p.date
      ,to_char(p.date_conception, 'Mon DD, YYYY') AS date_conception
      ...
      ,to_char(p.date_due_early, 'Mon DD, YYYY') || ' to ' || to_char(p.date_due_late, 'Mon DD, YYYY') AS date_due_range
      ,timestampdiff('SQL_TSI_DAY', p.date_conception_late, coalesce(po.date, curdate())) || ' to ' || timestampdiff('SQL_TSI_DAY', p.date_conception_early, coalesce(po.date, curdate())) || ' days' AS gestation_day_range
      ,to_char(p.date_due, 'Mon DD, YYYY') AS date_due
      ,timestampdiff('SQL_TSI_DAY', p.date_conception, coalesce(po.date, curdate())) || ' days' AS gestation_day
      ,po.outcome
