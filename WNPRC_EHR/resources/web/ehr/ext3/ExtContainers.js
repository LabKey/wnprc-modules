...
//                    })
    ...
    getAnimals: function(button)
    {
        ...
        if(subjectList){
            subjectList = LDK.Utils.splitIds(subjectList, true, false);
...
EHR.ext.ChemExcelWin = Ext.extend(Ext.Panel, {
    ...
    doSubmit: function(button)
    {
        ...
        Ext.Ajax.request({
            ...
            params: {
                fileName: 'ChemistryUpload_'+(new Date()).format(LABKEY.extDefaultDateFormat + '_H:m:s')+'.tsv',
 ...
 * in the chemistry section of the ClinPath form.
 */
EHR.ext.ChemVetAxcelWin = Ext.extend(Ext.Panel, {
    initComponent: function()
    {
        ...
        jQuery.each(runsStore, function(i,resultRow){
            ids += resultRow.get('Id')+ ";";
        ...
        Ext.applyIf(this, {
            //layout: 'form'
            title: 'Import Chemistry Results from Vet Axcel'
            ,bodyBorder: true
            ,border: true
            //,frame: true
            ,bodyStyle: 'padding:5px'
            ...
            ,items: [{
                xtype: 'displayfield',
                value: 'This windows will show Chemistry Results imported from the Chemistry Analyzer'
            },{
                xtype: 'textarea',
                ref: 'fileField',
                height: 350,
    ...
    onSuccess :function(results){
        ...
        if (!results.rows || !results.rows.length){
            alert('No matching results from animals in ClinPath Run');
            return;
        }else {
    ...
    doSubmit: function(button)
    {
        ...
        //this.targetStore.addRecords(window.storeResults);
...
Ext.reg('ehr-chemvetaxcelwin', EHR.ext.ChemVetAxcelWin);
