...
//                        layout: 'form',
        ...
        addchemvetaxcel: {
            text: 'Add From Vet Axcel',
            //requiredQC: 'In Progress',
            xtype: 'button',
            scope: this,
            tooltip: 'Click to add results from a Vet Axcel LIMS',
            name: 'add-blood-button',
            handler: function()
            {
                this.selectorWin = new Ext.Window({
                    ...
                    width: 450,
                    items: [{
                        xtype: 'ehr-chemvetaxcelwin',
                        ref: 'chemaxcelselector',
                        targetStore: this.store,
                        parentPanel: this,
                        title: ''
                    }]
                                            ...
                                            jQuery.each(data.rows, function(i, resultRow) {
                                                ...
                                                jQuery.each(clinpathRecords, function(i, clinpathRun) {
                                                    if (clinpathRun.get('Id') == resultRow['Id']
                                                     && clinpathRun.get('date').format('Y-m-d') == new Date(resultRow['Date']).format('Y-m-d')
