 ...
 * Time: 3:49 PM
 */
public class AgeMonthsDisplayColumn extends DataColumn
{
    ...
    public static String getFormattedAge(Date birth, Date death)
    {
        ...
        double yearPart = (deathCal.getTimeInMillis() - birthCal.getTimeInMillis()) / (DateUtils.MILLIS_PER_DAY * 30.4375);
        int yearRounded = (int)yearPart;
        birthCal.set(Calendar.MONTH, birthCal.get(Calendar.MONTH) + yearRounded);
        ...
        return yearRounded + " month" + (yearRounded == 1 ? "" : "s") + ", " + dayPart + " day" + (dayPart == 1 ? "" : "s");
    }
}
